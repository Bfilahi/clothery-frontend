...
export class CartDetailsComponent {
  ...
  public getAvailableQuantities(item: CartItem): number[]{
    const selectedSize = item.sizes.find(sq => sq.size === item.selectedSize);

    if(selectedSize)
      return Array.from({length: selectedSize.quantity}, (_, i) => i + 1);
